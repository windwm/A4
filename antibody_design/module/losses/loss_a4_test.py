...
class PromptLossNet(nn.Cell):
    ...
    def construct(self, bert_aa_logit, bert_fragment_logit, 
        ...
        ##fragment_label = mnp.clip(ab_label, 0, self.fragment_bins-1) ### 避免标签越界
...
class GenerationLossNet(nn.Cell):
    ...
    def construct(self, log_probs_aa_prior, beit_log_probs_aa, log_probs_aa_posterior,
                  label_aa,
                  label_mask):
        """construct"""
        ...
        loss_aa_beit = self.loss_xent_func(beit_log_probs_aa, ab_label_onehot, label_mask)
        loss_aa_posterior = self.loss_xent_func(log_probs_aa_posterior, ab_label_onehot, label_mask)

        ### 3. 在withlosscell里根据config.train按权重组合各项loss
        return loss_aa_prior, loss_aa_posterior, loss_aa_beit


# ### 20230331 added by @ZhangJ.
# class PairLossNet(nn.Cell):
#     """loss net for chain-pairing model"""

#     def __init__(self, config):
#         super(PairLossNet, self).__init__()
#         self.config = config.train.generation_model
...
#         self.neg_index = config.data.prompt_model.neg_index        
#         self.aa_bins = config.data.prompt_model.aa_types
#         self.negative_index = config.data.generation_model.negative_index

#         self.aa_onehot = nn.OneHot(depth=self.aa_bins, axis=-1)
#         self.aa_xent = MultiClassFocalLoss(num_class=self.aa_bins, beta=0.99, gamma=self.focal_gamma, e=self.label_smoothing, not_focal=self.not_focal_loss, balanced=self.focal_balance)
        
#         self.logsumexp = nn.ReduceLogSumExp(axis=-1, keep_dims=True)
#         self.infonce_temperature = self.config.infonce_temperature
#         self.if_run_posterior = self.config.if_run_posterior
...
#     def loss_contrastive_func(self, loss_xent, seq_mask):
#         """Contrastive Loss"""
#         ### loss_xent: (B,S); seq_mask:(B,S)
#         # loss_xent = mnp.clip(loss_xent, 0.0, 3.0) # 
        
#         ### @ZhangJ. bug: LogSoftmaxGrad报错 不支持>2维的tensor 
#         scores = - loss_xent / self.infonce_temperature
#         # padding scores with large negatives:
#         # (B,S):
#         scores += (seq_mask - 1) * 1e3
        
#         # (B,1):
#         scores_pos = scores[:,:self.negative_index]
        
#         # (B,1):
#         loss = self.logsumexp(scores) - scores_pos # InfoNCE loss
...
#         ### 1. 计算loss
#         ab_label = mnp.clip(label_aa, 0, self.aa_bins-1) ### 避免标签越界
#         ab_label_onehot = self.aa_onehot(ab_label)  # bs, nseq, nres, bins 
        
#         neg_label = ab_label_onehot[:, self.negative_index:, :]
#         neg_label_ones = ops.ones_like(neg_label) * 1 / self.aa_bins
#         ab_label_onehot = ops.concat([ab_label_onehot[:, :self.negative_index, :], neg_label], axis=1)
...
#         # 2. 计算contrastive loss:
#         # (B,S):
#         seq_mask = mnp.clip(mnp.sum(label_mask, axis=-1), 0., 1.)
#         loss_xent = loss_aa_prior
#         if self.if_run_posterior:
#             loss_xent = loss_aa_posterior
#         # (B,1):
#         loss_contrastive = self.loss_contrastive_func(loss_xent, seq_mask)
        
#         # (B,1):
#         loss_aa_prior = loss_aa_prior[:,:self.negative_index]
#         loss_aa_posterior = loss_aa_posterior[:,:self.negative_index]

#         ### 3. 在withlosscell里根据config.train按权重组合各项loss
#         return loss_aa_prior, loss_aa_posterior, loss_contrastive
...
class PairLossNet(nn.Cell):
    ...
    def __init__(self, config):
        ...
        self.if_run_posterior = self.config.if_run_posterior
        self.cosent_lambda = config.cosent_lambda
    ...
    def loss_cs_func(self, score, mask):
        ...
        # (B,P):
        sp = score[:,:self.negative_index]
        positive_mask = mask[:,:self.negative_index]
        # (B,N):
        sn = score[:,self.negative_index:]
        negative_mask = mask[:,self.negative_index:]
        
        # (B,P):
        logit_p = sp * self.cosent_lambda
        ### @ZhangJ. pos需要padding:
        logit_p = logit_p*positive_mask + (1. - positive_mask)*1e3 ### 注意与negative padding的不同
        
        # (B,N):
        logit_n = sn * self.cosent_lambda
        ### @ZhangJ. 需要padding;
        logit_n = logit_n*negative_mask + (negative_mask - 1.)*1e3 ### 注意与positive padding的不同

        ### @ZhangJ. 采用numerically stable的方式计算circle loss:
        ### c.f. Eq.(4) & Eq.(6) in Circle Loss Paper.
        term = self.logsumexp(-logit_p) + self.logsumexp(logit_n) # (B,1)
        errors = self.softplus(term) # (B,1)
    ...
    def construct(self, log_probs_aa_prior, beit_log_probs_aa, log_probs_aa_posterior,
        ...
        '''
        # (B,1):
        loss_contrastive = loss_contrastive_func(loss_xent, seq_mask)
        '''
        # (B,S):
        score = - mnp.clip(loss_xent, 0., 4.)
        # (B,1):
        loss_contrastive = self.loss_cs_func(score, seq_mask)
        
        # (B,1):
        loss_aa_prior = loss_aa_prior
        loss_aa_posterior = loss_aa_posterior
...
class AffinityLossNet(nn.Cell):
    ...
    def meta_normalization(self, errors, label_mask, target_mask):
        ...
        ### 在一个样本内对target序列做加和：
        ### 在batch所有样本上对label做加和（有些样本没有这个label）:
