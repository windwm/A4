...
### @ZhangJ. ToDo: Binary & MultiClass Focal Loss:
class MultiClassFocalLoss(nn.Cell):
    ...
    def construct(self, prediction_logits, target_tensor):
        ...
        # reference : https://spaces.ac.cn/archives/4493
        _labels = target_tensor
        # (None,), (None,100):
        xent, _dlogits = self.cross_entropy(prediction_logits, target_tensor)
        ...
        if self.not_focal:
            # (None,), (None,100):
            softmax_xent, _dlogits = self.cross_entropy(prediction_logits, _labels)
